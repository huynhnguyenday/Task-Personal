            ...
            {data.map((person, index) => {
              ...
              return (
                <li key={person.name} className="overflow-hidden border border-transparent bg-[#e3f0e9] data-[expanded=true]:border-[#a8c9bc]" data-expanded={expanded}>
                  <button className="relative grid min-h-14 w-full grid-cols-[20px_minmax(0,1fr)_38px_12px] items-center gap-2 overflow-hidden px-2 text-left sm:grid-cols-[20px_minmax(120px,1fr)_minmax(140px,0.85fr)_44px_12px] sm:gap-3 sm:px-3" type="button" aria-expanded={expanded} onClick={() => setExpandedName(expanded ? null : person.name)}>
                    ...
                    <span className="relative hidden min-w-0 truncate text-center text-xs font-bold text-[#3f494f] sm:block">{departments}</span>
                  ...
                  {expanded && (
                    <div className="grid gap-1.5 border-t border-[#dce6e1] bg-white p-2">
                      {person.tasks.map((task, taskIndex) => (
                        <button key={task.id} className="grid grid-cols-[20px_minmax(0,1fr)_72px] items-center gap-2 bg-[#e5efea] px-2 py-2.5 text-left transition hover:bg-[#bfd8cd] focus:outline-none focus:ring-1 focus:ring-inset focus:ring-[#28745b] sm:grid-cols-[24px_minmax(120px,1fr)_minmax(130px,0.85fr)_88px] sm:px-2.5 sm:py-2" type="button" onClick={() => setSelectedTask(task)}>
                          ...
                          <span className="hidden min-w-0 truncate text-center text-[10px] font-bold text-[#515a60] sm:block">{task.department || "Chưa có phòng ban"}</span>
