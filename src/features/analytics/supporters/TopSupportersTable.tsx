            ...
            {data.map((person, index) => {
              ...
              return (
                <li key={person.name} className="overflow-hidden border border-transparent bg-[#e3f0e9] data-[expanded=true]:border-[#a8c9bc]" data-expanded={expanded}>
                  <button className="relative grid min-h-14 w-full grid-cols-[20px_minmax(120px,1fr)_minmax(140px,0.85fr)_44px_12px] items-center gap-3 overflow-hidden px-3 text-left" type="button" aria-expanded={expanded} onClick={() => setExpandedName(expanded ? null : person.name)}>
                  ...
                  {expanded && (
                    <div className="grid gap-1.5 border-t border-[#dce6e1] bg-white p-2">
                      {person.tasks.map((task, taskIndex) => (
                        <button key={task.id} className="grid grid-cols-[24px_minmax(120px,1fr)_minmax(130px,0.85fr)_88px] items-center gap-2 bg-[#e5efea] px-2.5 py-2 text-left transition hover:bg-[#bfd8cd] focus:outline-none focus:ring-1 focus:ring-inset focus:ring-[#28745b]" type="button" onClick={() => setSelectedTask(task)}>
