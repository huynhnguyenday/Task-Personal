            ...
            {data.map((person, index) => {
              ...
              return (
                <li key={person.name} className="overflow-hidden border border-transparent bg-[#e3f0e9] data-[expanded=true]:border-[#a8c9bc]" data-expanded={expanded}>
                  <button className="relative grid min-h-14 w-full grid-cols-[20px_minmax(0,1fr)_minmax(100px,1fr)_auto_12px] items-center gap-3 overflow-hidden px-3 text-left" type="button" aria-expanded={expanded} onClick={() => setExpandedName(expanded ? null : person.name)}>
                    ...
                    <span className="relative min-w-0 truncate text-center text-xs font-bold text-[#3f494f]">{departments}</span>
                    <strong className="relative text-sm text-[#28745b]">{person.count}</strong>
                  ...
                  {expanded && (
                    <div className="grid gap-1.5 border-t border-[#dce6e1] bg-white p-2">
                      {person.tasks.map((task, taskIndex) => (
                        <button key={task.id} className="grid grid-cols-[24px_minmax(0,1.4fr)_minmax(90px,1fr)_auto] items-center gap-2 bg-[#e5efea] px-2.5 py-2 text-left transition hover:bg-[#bfd8cd] focus:outline-none focus:ring-1 focus:ring-inset focus:ring-[#28745b]" type="button" onClick={() => setSelectedTask(task)}>
                          <span className="text-[10px] font-bold text-[#28745b]">{String(taskIndex + 1).padStart(2, "0")}</span>
                          <strong className="block min-w-0 truncate text-xs font-bold text-[#3f494f]">{task.description}</strong>
                          <span className="min-w-0 truncate text-center text-[10px] font-bold text-[#515a60]">{task.department || "Chưa có phòng ban"}</span>
                          <span className="text-[10px] font-semibold text-[#515a60]">{formatDate(task.createdAt)}</span>
                        </button>
                      ))}
                    </div>
                  )}
