...
export default function WorkloadTasksModal({ type, label, count, onClose }: { type: WorkloadFilterType; label: string; count: number; onClose: () => void }) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[90] flex items-end justify-center bg-[#17251d]/60 p-0 backdrop-blur-[2px] sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="workload-tasks-title" onMouseDown={onClose}>
        <section className="flex h-[92dvh] w-full max-w-[1040px] flex-col overflow-hidden bg-[#f5f7f5] shadow-[0_24px_70px_rgba(20,35,29,0.3)] sm:h-[min(86dvh,780px)]" onMouseDown={(event) => event.stopPropagation()}>
          <header className="flex shrink-0 items-start justify-between gap-3 border-b border-[#e3e7e9] bg-white px-4 py-4 sm:px-6">
            <div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[1.4px] text-[#28745b]">{type === "department" ? "PHÒNG BAN" : "DANH MỤC"}</p><h2 id="workload-tasks-title" className="mt-1 truncate text-lg font-semibold sm:text-xl">{label}</h2><p className="mt-1 text-xs text-[#727a82]">{count} công việc</p></div>
          ...
          <div className="grid shrink-0 gap-2 border-b border-[#e3e7e9] bg-white p-3 sm:grid-cols-2 sm:p-4">
          ...
          <div className="scrollbar-analytics min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
            ...
            {loading ? <div className="grid gap-2">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-[86px] w-full" />)}</div> : tasks.length ? <div className="grid gap-2">{tasks.map((task) => (
              <button key={task.id} className={`grid w-full grid-cols-[minmax(0,1fr)_auto] gap-3 border-l-4 p-3 text-left transition hover:-translate-y-px hover:shadow-sm sm:grid-cols-[minmax(0,1.7fr)_minmax(130px,.8fr)_minmax(110px,.65fr)_auto] sm:items-center sm:px-4 ${statusTone(task.status)}`} type="button" onClick={() => setSelectedTask(task)}>
                ...
                <div className="hidden min-w-0 sm:block"><span className="block text-[9px] font-bold uppercase tracking-wide text-[#727a82]">Người hỗ trợ</span><strong className="mt-1 block truncate text-xs text-[#3f494f]">{task.supportPerson || "—"}</strong></div>
                <div className="hidden sm:block"><span className="block text-[9px] font-bold uppercase tracking-wide text-[#727a82]">Ngày tạo</span><time className="mt-1 block text-xs font-bold text-[#3f494f]">{formatDate(task.createdAt)}</time></div>
                <FontAwesomeIcon className="self-center text-xs text-[#28745b]" icon={faChevronRight} />
                <div className="col-span-2 flex items-center justify-between text-[10px] font-bold sm:hidden"><span>{task.supportPerson || "—"}</span><time>{formatDate(task.createdAt)}</time></div>
              </button>
