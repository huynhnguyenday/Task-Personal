...
export default function OverviewCard({ label, value, icon, loading, error, tone }: { label: string; value: number; icon: IconDefinition; loading: boolean; error: string; tone: string }) {
  ...
  return (
    <article className="relative flex min-h-[88px] min-w-0 items-center gap-2 overflow-hidden border border-[#e3e7e9] bg-white px-2 py-2 sm:gap-3 sm:px-3">
      <span className={`grid h-9 w-9 shrink-0 place-items-center ${tone}`}><FontAwesomeIcon icon={icon} /></span>
      <div className="min-w-0 flex-1 text-center">
        <p className="line-clamp-2 min-h-8 text-xs font-bold leading-4 text-[#3f494f] sm:text-sm">{label}</p>
