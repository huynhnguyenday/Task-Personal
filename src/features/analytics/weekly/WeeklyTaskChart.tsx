...
export default function WeeklyTaskChart() {
  ...
  useEffect(() => {
    ...
    const cacheKey = `analytics:weekly:${appliedRange.from}:${appliedRange.to}`;
  ...
  return (
    <article className="flex h-full min-h-0 flex-col border border-[#e3e7e9] bg-white p-3.5 sm:p-4">
      <div className="shrink-0">
        <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-bold tracking-[1.4px] text-[#28745b]">THEO KHOẢNG NGÀY</p><h2 className="mt-1 text-lg font-semibold">Số lượng task</h2></div><button className={`h-9 shrink-0 border px-3 text-xs font-bold transition ${showValues ? "border-[#28745b] bg-[#e3f0e9] text-[#28745b]" : "border-[#d9dfe0] bg-white text-[#515a60]"}`} type="button" aria-pressed={showValues} onClick={toggleValues}>{showValues ? "Ẩn số" : "Hiện số"}</button></div>
        <form className="mt-3 grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] items-end gap-2" onSubmit={applyFilter}>
          <label className="grid min-w-0 gap-1 text-[10px] font-bold text-[#515a60]">TỪ NGÀY<input className="h-9 min-w-0 border border-[#d9dfe0] bg-[#fafbfa] px-2 text-xs font-semibold outline-none focus:border-[#28745b]" type="date" value={from} max={to} onChange={(event) => setFrom(event.target.value)} required /></label>
          <label className="grid min-w-0 gap-1 text-[10px] font-bold text-[#515a60]">ĐẾN NGÀY<input className="h-9 min-w-0 border border-[#d9dfe0] bg-[#fafbfa] px-2 text-xs font-semibold outline-none focus:border-[#28745b]" type="date" value={to} min={from} onChange={(event) => setTo(event.target.value)} required /></label>
