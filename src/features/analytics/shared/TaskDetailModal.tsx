...
function TaskDetailContent({ task, onClose, onTaskUpdated }: { task: TaskDetail; onClose: () => void; onTaskUpdated?: (task: TaskDetail) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center overflow-hidden bg-[#17251d]/60 p-2 backdrop-blur-[2px] sm:p-4" role="dialog" aria-modal="true" aria-labelledby="task-detail-title" onMouseDown={() => { if (!saving) onClose(); }}>
      <section className="scrollbar-analytics max-h-[calc(100dvh-1rem)] w-full min-w-0 max-w-[720px] overflow-x-hidden overflow-y-auto bg-white shadow-[0_24px_70px_rgba(20,35,29,0.3)] sm:max-h-[90dvh]" onMouseDown={(event) => event.stopPropagation()}>
        <header className="sticky top-0 z-10 flex min-w-0 items-start justify-between gap-2 border-b border-[#e3e7e9] bg-white px-4 py-4 sm:gap-4 sm:px-7 sm:py-5">
          <div><h2 id="task-detail-title" className="mt-1 text-sm font-semibold">{editing ? "Chỉnh sửa task" : "Thông tin task"}</h2></div>
        ...
        {editing ? (
          <form className="min-w-0 p-4 sm:p-7" onSubmit={save}>
          ...
          <div className="min-w-0 p-4 sm:p-7">
            <div className="min-w-0 border-l-4 border-[#28745b] bg-[#f5f7f5] p-4"><p className="text-[10px] font-bold uppercase tracking-[1.2px] text-[#515a60]">Mô tả công việc</p><p className="mt-2 whitespace-pre-wrap break-words [overflow-wrap:anywhere] text-[15px] font-medium leading-6 text-[#20252b]">{task.description}</p></div>
            <div className="mt-5 grid min-w-0 gap-3 sm:grid-cols-2">{detailFields.map((detail) => <div key={detail.key} className="min-w-0 border border-[#e3e7e9] p-3.5"><p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[1px] text-[#515a60]"><FontAwesomeIcon className="shrink-0 text-[#28745b]" icon={detail.icon} />{detail.label}</p><p className="mt-2 break-words [overflow-wrap:anywhere] text-sm font-semibold">{task[detail.key] || "—"}</p></div>)}<div className="min-w-0 border border-[#e3e7e9] p-3.5"><p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[1px] text-[#515a60]"><FontAwesomeIcon className="shrink-0 text-[#28745b]" icon={faCalendarDay} />Ngày tạo</p><p className="mt-2 text-sm font-semibold">{createdAt}</p></div></div>
            ...
            <div className="mt-5 min-w-0"><p className="text-[10px] font-bold uppercase tracking-[1.2px] text-[#515a60]">Ghi chú</p><p className="mt-2 min-h-16 whitespace-pre-wrap break-words [overflow-wrap:anywhere] border border-[#e3e7e9] bg-[#fafbfa] p-4 text-sm font-medium leading-6 text-[#3f494f]">{task.notes || "Không có ghi chú."}</p></div>
          </div>
        )}
      </section>
    </div>
